...
hists = data['hists']
means = np.expand_dims(data['means'], -1)
stds = data['stds']
filenames = data['filenames']

ids = range(means.shape[0])

label_np = np.empty(shape=(len(ids), 2),dtype=float)
...
class DataSequence(Sequence):
    ...
    def __init__(self, label_np, data_path, batch_size, mode='train'):
        ...
        self.im_list = [os.path.join(data_path, filenames[int(x)]) for x in self.df[:,0]]
...
distribution_elements_row = tf.constant(np.array(range(100,1100))/100., dtype='float32', name='marks')
...
def pearson_corr(means_true, means_pred):
...
model.compile(loss='mse', optimizer=sgd, metrics=['mse', pearson_corr]) 

# checkpoint
checkpoint = ModelCheckpoint('weights-tid2013-regression-{epoch:02d}-{pearson_corr:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
sgd = keras.optimizers.Adam(lr=1e-5)
...
train_seq = DataSequence(label_train, data_path,  batch_size=batch_size)
test_seq = DataSequence(label_test, data_path,  batch_size=batch_size, mode='val')
...
# see the Readme what lr to use there !
sgd = keras.optimizers.Adam(lr=1e-7)


model.compile(loss='mse', optimizer=sgd, metrics=['mse', pearson_corr]) 

# checkpoint. Save all checkpoins as validation does not improve !!
checkpoint = ModelCheckpoint('weights-tid2013-regression-continue2-{epoch:02d}-{pearson_corr:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)
...
print('loss: {}, corr:{}'.format(scores[0], scores[2])) # EMD loss: 0.07736995112838584, bin_acc:0.7642938206957468, chi-sq:7.629008602695736
